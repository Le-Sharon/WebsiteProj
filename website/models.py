...
User_Carts = db.Table('user_carts',
    db.Column('userid', db.Integer, db.ForeignKey('user.id')),
...
class Cart(db.Model):
    ...
    users = db.relationship('User', secondary=User_Carts, backref=db.backref('user_carts', lazy=True))
    games = db.relationship('Game', secondary=Cart_Games, backref=db.backref('game_carts', lazy=True))
    
    @property
    def subtotal(self):
        return db.session.query(func.sum(Game.price)).filter(Game.carts.contains(self)).scalar() or 0.0
...
class Order(db.Model):
    ...
    @property
    def games(self):
        return self.cart.games #returns all games in the cart
